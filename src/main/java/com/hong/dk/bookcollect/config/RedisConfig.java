...
public class RedisConfig {
    ...
    public KeyGenerator keyGenerator() {
        return new KeyGenerator() { // 设置自定义的key规则
            @Override
            public Object generate(Object target, Method method, Object... params) { // target就是被调用的对象，method是被调用的方法，params是方法的参数
                StringBuilder sb = new StringBuilder(); // 定义一个StringBuilder字符串
                sb.append(target.getClass().getName()); // 取出类名
                sb.append(method.getName()); // 取出方法名
                for (Object obj : params) {
                    sb.append(obj.toString()); // 取出参数
                }
                return sb.toString();
            }
        };
